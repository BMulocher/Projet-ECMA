...
colgenSolver::colgenSolver() : Solver::Solver() {
    name = "colgenSolver";
    desc = "Génération de colonnes à partir du résultat de l'heuristique gloutonne\n";
}
...
void
colgenSolver::setcolgen (Data data, Solution* sol, IloModel model, BoolVarMatrix x, NumVarMatrix a, NumVarMatrix p, BoolVarTriMat d, vector<vector<int>> deleted_variables, IloRangeArray constraints) {
  ...
  // Contraintes de nullité de variables pour la génération de colonnes
  auto it = deleted_variables[0].begin();
  while( it != deleted_variables[0].end() ) 
  {
      constraints.add(x[deleted_variables[0][*it]][deleted_variables[1][*it]]==0);
      constraints.add(a[deleted_variables[0][*it]][deleted_variables[1][*it]]==0);
      constraints.add(p[deleted_variables[0][*it]][deleted_variables[1][*it]]==0);
      it++;
  }
...
//
bool colgenSolver::solve(Data data, Solution* sol) {
    if (log2()) {
        cout << "\n------ colgenSolver::solve START ------\n";
    }
    ...
    if (solver->found) {
      ...
      try {
          ...
          vector<vector<int>> deleted_variables;
          vector<int> row;
          deleted_variables.push_back(row);
          deleted_variables.push_back(row);          
          ...
          for (int i = 1; i <= data.m; i++)
          {
            for (int j = 1; j <= data.n; j++)
            {
              if (sol->x[i][j]==0)
              {
                deleted_variables[0].push_back(i);
                deleted_variables[1].push_back(j);
              }
            }
          }

          // Set modèle
          setcolgen(data, sol, model, x, a, p, d, deleted_variables, constraints);
